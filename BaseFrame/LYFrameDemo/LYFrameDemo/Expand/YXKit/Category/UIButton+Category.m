...
static NSString *const ButtonImageObjectKey = @"buttonImageObject";
    ...
    dispatch_source_set_event_handler(_timer, ^{
        ...
        if (timeOut <= 0) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [weakSelf setTitle:[NSString stringWithFormat:@"已发送(%@%@)",timeStr,subTitle] forState:UIControlStateNormal];
...
- (void)showIndicator
{
    ...
    objc_setAssociatedObject(self, &ButtonTextObjectKey, self.currentTitle, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    objc_setAssociatedObject(self, &ButtonImageObjectKey, self.currentImage, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)hideIndicator
{
    ...
    UIImage *currentButtonImage = (UIImage *)objc_getAssociatedObject(self, &ButtonImageObjectKey);
